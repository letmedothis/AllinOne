...
public class DocumentLifecycleServiceImpl implements IDocumentLifecycleService {
    ...
    @Override @Transactional public int withdraw(Long id, String reason) { DocumentLifecycle d=load(id); requireReason(reason); requireOwner(d); if(!"IN_REVIEW".equals(d.getStatus()))throw new ServiceException("只有审批中的单据可以撤回"); mapper.cancelTasks(id); mapper.releaseAllocations(id); cancelFlowable(d); mapper.updateWorkflow(id,"WITHDRAWN"); return change(d,"RETURNED","WITHDRAW",reason); }
    @Override @Transactional public int voidDocument(Long id, String reason) { DocumentLifecycle d=load(id); requireReason(reason); if("INVOICE".equals(d.getType()) && mapper.countInvoicePayments(id)>0) throw new ServiceException("发票已有审批中或已生效付款，请先处理付款或通过受控更正流程办理"); if(!"SUPPLIER".equals(d.getType())&&!"PURCHASE_ORDER".equals(d.getType())&&!"INVOICE".equals(d.getType()))throw new ServiceException("入库单不支持作废，请按后续更正流程处理"); if("DRAFT".equals(d.getStatus())||"RETURNED".equals(d.getStatus())){requireOwner(d);}else if("APPROVED".equals(d.getStatus())){if("SUPPLIER".equals(d.getType())||"PURCHASE_ORDER".equals(d.getType())){if(!SecurityUtils.isAdmin()&&!hasRole("supervisor")&&!hasRole("purchasing_supervisor")&&!hasRole("purchase_supervisor"))throw new ServiceException("已通过供应商或订单仅采购主管或管理员可作废");}else if("INVOICE".equals(d.getType())){if(!SecurityUtils.isAdmin()&&!hasRole("finance"))throw new ServiceException("已通过发票仅财务或管理员可作废");if(!SecurityUtils.isAdmin()&&SecurityUtils.getUserId().equals(d.getCreatorId()))throw new ServiceException("发票提交人不能作废本人已通过发票");}if("SUPPLIER".equals(d.getType())&&mapper.countSupplierReferences(id)>0)throw new ServiceException("供应商已有有效订单，不能作废");if("PURCHASE_ORDER".equals(d.getType())&&(mapper.countOrderReceipts(id)>0||mapper.countOrderInvoices(id)>0))throw new ServiceException("订单已有下游业务，不能作废");}else throw new ServiceException("当前状态不允许作废"); mapper.cancelTasks(id); cancelFlowable(d); if("INVOICE".equals(d.getType()))mapper.releaseAllocations(id); return change(d,"VOID","VOID",reason); }
    ...
    private void cancelFlowable(DocumentLifecycle d) {
        if (!"FLOWABLE".equals(d.getWorkflowEngine())) return;
        for (ProcessInstance instance : runtimeService.createProcessInstanceQuery()
                .processInstanceBusinessKey(String.valueOf(d.getDocumentId()))
                .processDefinitionKey(WorkflowEngineServiceImpl.PURCHASE_ORDER_PROCESS_KEY).list()) {
            runtimeService.deleteProcessInstance(instance.getId(), "DOCUMENT_LIFECYCLE");
        }
    }
