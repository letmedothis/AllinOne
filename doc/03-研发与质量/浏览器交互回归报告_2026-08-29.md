...
![修复后设计器正常加载](../assets/gui-test-screenshots/r2_designer.png)

![公式计算（SUM=300、直接引用=300）](../assets/gui-test-screenshots/r3_final.png)

![图表插入成功（echarts 双系列折线图）](../assets/gui-test-screenshots/r9d_chart_panel.png)

![填报提交成功](../assets/gui-test-screenshots/r10_submitted.png)

![详情快照回放一致](../assets/gui-test-screenshots/r13_replay_h2.png)

![报表查看 iframe 携带 ticket 正常渲染](../assets/gui-test-screenshots/r14_report_view.png)

![生产构建设计器正确还原模板数据与图表](../assets/gui-test-screenshots/r17_prod_designer_data.png)
...
- Docker 中重建 `mysql`(8.0, 端口 3306) 与 `redis`（数据卷 `/app/mysql/data` 未动）；按 `doc/04-部署与运维/SQL_EXECUTION_ORDER.md` 顺序导入全部 6 个 SQL（90 张表），通过 `@bootstrap_password_bcrypt` 设置本地 admin 密码。
...
| # | 测试点 | 证据 |
|---|--------|------|
| T1 | 登录页渲染、数学验证码、admin 登录成功跳转 /index | ![登录](../assets/gui-test-screenshots/t2_login_page.png) |
| T2 | 首页/侧边栏/页签渲染，中文字体正常 | ![首页](../assets/gui-test-screenshots/t2_after_login.png) |
| T3 | 业务管理→填报模板列表、新增模板弹窗校验与创建 | ![模板列表](../assets/gui-test-screenshots/t3_template_list.png) |
| T4 | 模板编辑页框架（名称/编码/分类表单、返回/保存/发布按钮） | ![编辑页框架](../assets/gui-test-screenshots/t4g_reload.png) |
| T5 | 模板发布：确认框文案正确，状态 未发布→已发布，操作变下架 | ![发布确认](../assets/gui-test-screenshots/t6b_confirm_box.png) |
| T6 | 填报数据列表、新增填报页、模板下拉正确加载已发布模板 | ![填报页](../assets/gui-test-screenshots/t6e_after_pick.png) |
| T7 | 报表配置列表/新增弹窗/保存成功；查看页对无效 jimu id 正确报"报表不存在或已停用" | ![配置创建](../assets/gui-test-screenshots/t8b_saved_list.png) |
| T8 | 报表管理（work_report）列表页正常加载 | ![报表管理](../assets/gui-test-screenshots/t9_workreport_list.png) |
...
![dev 设计器失败](../assets/gui-test-screenshots/t4g_reload.png)

![prod 设计器失败](../assets/gui-test-screenshots/t10b_prod_designer.png)
...
- 截图证据：`doc/assets/gui-test-screenshots/`（21 张，t1–t10 编号与测试点对应）
