...
**已知遗留（不影响主链路，建议后续处理）：**
1. **图表跨页面重开不还原**：图表插入、保存、库内持久化均正常，但重新打开设计器/详情时图表不渲染（数据仍在 JSON 中）。根因是本地构建把 chartmix 拆为 create 内部异步 CDN 加载，与数据图表的同步渲染存在时序竞争，属上游 luckysheet 架构问题；官方发行版将 chartMix 打进 plugin.js 同步加载，无此问题。彻底修法是调整本地构建让 chartmix 同步打包。
2. chartmix 及其依赖（vue2/vuex/element-ui/echarts4）走公网 CDN（jsdelivr/unpkg/bootcdn），离线/内网环境图表功能不可用；print 插件硬编码 `localhost:8080`。
3. 提交后页面状态切换偶发上游 pageerror `reading 'chart_id'`（sheetmanage 图表列表重建的边缘 case），瞬态、不影响提交与回放。
