<template>
  <div class="app-container">
    ...
    <el-empty v-if="!loading && !list.length" description="当前没有待处理的审批任务" />
...
function decide(task: ApprovalTask, isApprove: boolean) {
  current.value = task
  approving.value = isApprove
  comment.value = ''
  dialogVisible.value = true
}
async function submitDecision() {
  if (!current.value) return
  if (!approving.value && !comment.value.trim()) {
    proxy.$modal.msgError('退回原因不能为空')
    return
  }
  const task = { ...current.value, taskRevision: current.value.revision, documentRevision: current.value.documentRevision, comment: comment.value }
  try {
    if (approving.value) await approve(task)
    else await reject(task)
    proxy.$modal.msgSuccess(approving.value ? '审批通过' : '已退回')
    dialogVisible.value = false
    getList()
  } catch (e: any) {
    proxy.$modal.msgError(e?.message || '审批操作失败')
  }
}
