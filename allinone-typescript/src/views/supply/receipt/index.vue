<template>
  <div class="app-container">
    <el-form :inline="true">
      <el-form-item label="待收货订单">
        <el-select v-model="form.orderId" filterable style="width: 400px" @change="onOrderChange">
          <el-option v-if="!orders.length && !optionsLoading" value="" label="暂无待收货订单" disabled />
          <el-option v-for="item in orders" :key="item.id" :value="item.id" :label="item.label" />
        </el-select>
      </el-form-item>
    ...
    <el-form v-if="lines.length" :model="form" label-width="100px" :disabled="busy">
      <el-form-item label="入库仓库">
        <el-select v-model="form.warehouseId" filterable>
          <el-option v-if="!warehouses.length && !optionsLoading" value="" label="暂无可选仓库" disabled />
          <el-option v-for="item in warehouses" :key="item.id" :value="item.id" :label="item.label" />
        </el-select>
      </el-form-item>
    ...
    <el-empty v-if="!busy && !history.length" description="暂无收货记录" />
...
import { confirmReceipt, listReceiptOrderOptions, listWarehouseOptions, listReceiptHistory, prepareReceipt } from '@/api/supply'
import type { SupplierOption } from '@/types/api/supply'
const { proxy } = getCurrentInstance()!
const form = reactive({ orderId: '', warehouseId: '', businessDate: '', remark: '' })
const lines = ref<any[]>([]), orders = ref<SupplierOption[]>([]), warehouses = ref<any[]>([]), history = ref<any[]>([])
const busy = ref(false), optionsLoading = ref(false), total = ref(0)
const query = reactive({ pageNum: 1, pageSize: 20 })
async function refreshOptions() {
  optionsLoading.value = true
  try {
    const [o, w] = await Promise.all([listReceiptOrderOptions(), listWarehouseOptions()])
    orders.value = o.data || []
...
function onOrderChange() {
  ...
  prepareReceipt(form.orderId).then(res => {
    lines.value = (res.data?.lines || []).map((line: any) => ({ ...line, quantity: 0 }))
  })
}
async function loadHistory() { const r = await listReceiptHistory(query); history.value = r.rows || []; total.value = r.total || 0 }
async function confirm() {
  ...
  busy.value = true
  try { await confirmReceipt({ ...form, lines: selected }); proxy.$modal.msgSuccess('入库确认成功'); await refreshOptions(); onOrderChange(); await loadHistory() } finally { busy.value = false }
}
